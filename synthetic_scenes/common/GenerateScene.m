...
function [P M m mnoiseless npoints] = GenerateScene(npoints, radius, ncams,...
    minz, maxz, noutliers, anoise, camcalib, sceneType,...
    pointsseed, camseed, rdcoefs, removeBehind, planarnoise)
...
if (nargin > 9) && (nargin <14) && ~isempty(pointsseed)
...
elseif strncmp(modelType, 'randomplanewithnoise', 20)
...
%     modelSize = 12;
    ...
    M(3, :) = 0+radius(1)*(rand()-0.5)+2*(rand(1,npoints)-0.5)*planarnoise ;
    
elseif strncmp(modelType, 'randomplane', 11)
    
    %generate 3D points
    modelSize = 11;
    M = 5+radius(1)*(rand(2, npoints) - 0.5);
     
    M(3, :) = 5+radius(1)*(rand()-0.5);
    for i = 5:5
        temp = M(:,2) - M(:,4);
...
elseif strncmp(modelType, 'sum_randomplane', 15)
    
    %generate 3D points
    modelSize = 11;
    M = radius(1)*(rand(2, npoints) - 0.5);
    M(3, :) = 0+0.01*rand(1,npoints)*radius(1);
    
elseif strncmp(modelType, 'rplane2', 7)
    
    %generate 3D points
    modelSize = 11;
    MM = radius(1)*(rand(2, npoints) - 0.5);
    MM(3, :) = 0;
    [U D V] = svd(rand(3,3));
    M = U*MM;
...
if nargin > 10 && (nargin <14) && ~isempty(camseed)
    randn('state',camseed);
end
...
if nargin > 13 && (nargin <14) && removeBehind
...
if nargin > 11 && (nargin <14) && ~isempty(rdcoefs)
